...
class E_STOP: # 클래스 1단계 : 클래스 이름
    ...
    def imu_CB(self, msg):
        ...
        imu_data = [f"{yaw_z} {imu_degree}"]

        if abs(self.pre_imu_degree - imu_degree) > 30:
            print(imu_degree)
            print("SAVE")
            self.pre_imu_degree = imu_degree
            self.csv_file.writerow(imu_data)
